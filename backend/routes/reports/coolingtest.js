...
router.get("/", async (req, res) => {
    ...
    try {
      ...
      const query1 = `SELECT model, barcode, StartTime, WorkStationNo, LineNo, PostNo, TestNo, TestedTime, TestResult, Remark FROM cooling_test WHERE StartTime >= '${startDate}' AND StartTime <= DATE_ADD('${endDate}', INTERVAL 1 DAY) order by CASE WHEN TestResult = 'NG' THEN 0 ELSE 1 END ASC, StartTime DESC`;
